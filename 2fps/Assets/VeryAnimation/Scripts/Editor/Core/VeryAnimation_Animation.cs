...
namespace VeryAnimation
{
    public partial class VeryAnimation
    {
        ...
        private Vector3 GetMirrorBoneLocalScale(int boneIndex, Vector3 localScale)
        {
            return localScale;
        }
        ...
        public Vector3 FixReverseRotationEuler(AnimationCurve[] curves, float time, Vector3 eulerAngles)
        {
            for (int i = 0; i < 3; i++)
            {
                ...
                if (curves[i] != null)
                {
                    ...
                    euler = curves[i].Evaluate(beforeTime);
                }
                while (Mathf.Abs(eulerAngles[i] - euler) > 180f)
                {
        ...
        public EditorCurveBinding? GetMirrorAnimationCurveBinding(EditorCurveBinding binding)
        {
            ...
            else if (IsSkinnedMeshRendererBlendShapeCurveBinding(binding))
            {
                ...
                string mirrorName;
                var name = binding.propertyName.Remove(0, "blendShape.".Length);
                if (!nameTable.TryGetValue(name, out mirrorName)) return null;
                binding.propertyName = "blendShape." + mirrorName;
                return binding;
            }
            else
            {
        ...
        public void SaveSelectionPoseTemplate(PoseTemplate poseTemplate)
        {
            ...
            #region BlendShape
            {
                ...
                {
                    var renderer = editBones[boneIndex].GetComponent<SkinnedMeshRenderer>();
                    if (renderer == null || renderer.sharedMesh == null || renderer.sharedMesh.blendShapeCount <= 0) continue;
                    var path = AnimationUtility.CalculateTransformPath(renderer.transform, editGameObject.transform);
                    var data = new PoseTemplate.BlendShapeData()
                    {
        ...
        private void SelectionCommonResetAll()
        {
            #region BlendShape
            {
                foreach (var boneIndex in selectionBones)
                {
                    ...
                    blendShapeWeightSave.ActionOriginalWeights(renderer, (name, value) =>
                    {
                        SetAnimationValueBlendShapeIfNotOriginal(renderer, name, blendShapeWeightSave.GetOriginalWeight(renderer, name));
                    });
                }
            }
            #endregion
        ...
        public void SetAnimationValueTransformPositionIfNotOriginal(int boneIndex, Vector3 position, float time = -1f)
        {
            ...
            if (IsHaveAnimationCurveTransformPosition(boneIndex) ||
                Mathf.Abs(position.x - boneSaveOriginalTransforms[boneIndex].localPosition.x) >= Threshold ||
                Mathf.Abs(position.y - boneSaveOriginalTransforms[boneIndex].localPosition.y) >= Threshold ||
                Mathf.Abs(position.z - boneSaveOriginalTransforms[boneIndex].localPosition.z) >= Threshold)
            {
        ...
        public void SetAnimationValueTransformRotationIfNotOriginal(int boneIndex, Quaternion rotation, float time = -1f)
        {
            ...
            if (IsHaveAnimationCurveTransformRotation(boneIndex) != URotationCurveInterpolation.Mode.Undefined ||
                Mathf.Abs(eulerAngles.x - originalEulerAngles.x) >= Threshold ||
                Mathf.Abs(eulerAngles.y - originalEulerAngles.y) >= Threshold ||
                Mathf.Abs(eulerAngles.z - originalEulerAngles.z) >= Threshold)
            {
        ...
        public void SetAnimationValueTransformScaleIfNotOriginal(int boneIndex, Vector3 scale, float time = -1f)
        {
            ...
            if (IsHaveAnimationCurveTransformScale(boneIndex) ||
                Mathf.Abs(scale.x - boneSaveOriginalTransforms[boneIndex].localScale.x) >= Threshold ||
                Mathf.Abs(scale.y - boneSaveOriginalTransforms[boneIndex].localScale.y) >= Threshold ||
                Mathf.Abs(scale.z - boneSaveOriginalTransforms[boneIndex].localScale.z) >= Threshold)
            {
        ...
        public float GetAnimationValueBlendShape(SkinnedMeshRenderer renderer, string name, float time = -1f)
        {
            if (currentClip == null || renderer == null || renderer.sharedMesh == null)
                return 0f;
            time = GetFrameSnapTime(time);
            float result = blendShapeWeightSave.GetOriginalWeight(renderer, name);
            {
                var curve = GetEditorCurveCache(AnimationCurveBindingBlendShape(renderer, name));
                if (curve != null)
                {

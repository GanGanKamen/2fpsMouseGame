            ...
            {
                {
                    ...
                    {
                        if (GUILayout.Button("?", toolsHelp ? guiStyleActiveButton : GUI.skin.button, GUILayout.Width(16), GUILayout.Height(14)))
                        {
                ...
                if (toolMode == ToolMode.ResetPose)
                {
                    ...
                    var activePrefab = PrefabUtility.GetCorrespondingObjectFromSource(Selection.activeGameObject) as GameObject;
#else
                    var activePrefab = PrefabUtility.GetPrefabParent(Selection.activeGameObject) as GameObject;
#endif
            ...
            try
            {
                #region Prefab
                {
                    for (int i = 0; i < prefabAssets.Length; i++)
                    {
                        ...
                        var saveSettings = gameObject.GetComponentsInChildren<VeryAnimationSaveSettings>();
                ...
                #region Scene
                {
                    for (int i = 0; i < sceneAssets.Length; i++)
                    {
                        ...
                        {
                            var saveSettings = go.GetComponentsInChildren<VeryAnimationSaveSettings>();
                            foreach (var cp in saveSettings)
                            {
                                Undo.DestroyObjectImmediate(cp);
                                updated = true;
